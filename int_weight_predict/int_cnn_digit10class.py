...
character_name = list('0123456789')
...
model_file_list_path = os.listdir(os.path.join(model_root_path, 'digitModel'))
...
def save_cnn_weight_to_bininary_file(model_weights):
    ...
    for weight in model_weights:
        print(weight.shape)
        # print(weight[0][0])
        fout = open(os.path.join(model_root_path, 'int_weights', 'digit10_int_weight%d.h' % count), 'w')
        fout.write('//%s\n' % str(weight.shape))
...
def save_img_to_bininary_file(test_X, test_y, name='val'):
    # 将图片保存成 二进制 形式
    with open(os.path.join(model_root_path, 'Data/images_digit10_%sdata.mat' % name), 'wb') as fout:
        print(test_X.shape)
    ...
    with open(os.path.join(model_root_path, 'Data/labels_digit10_%sdata.mat' % name),
              'wb') as fout:
...
def save_model_file_to_pickle(type=1):
    ...
    if type == 1:
        ...
        with open(
                '/home/jdwang/PycharmProjects/digitRecognition/int_weight_predict/modelAndData1214/digitModel/10class_model_weight.pkl',
                'w') as fout:
            ...
            model.load_weights(
                '/home/jdwang/PycharmProjects/digitRecognition/int_weight_predict/modelAndData1214/digitModel/iteration174_model_weights_10-40_region3_lr0.005_firstCNN_final.h5')
            pickle.dump(model.get_weights(), fout)
...
print(X_test.shape, X_other.shape, other_X_new.shape)
# quit()
# 真实值              ['0', '0', '0', '0', '0', '0', '0', 'C']
...
with open(os.path.join('/home/jdwang/PycharmProjects/digitRecognition/int_weight_predict/modelAndData1214/digitModel',
                       '10class_model_weight.pkl'), 'r') as fin:
    for index in range(1, len(model_file_list_path) + 1):
        # 从 模型1 开始，依次往后
        # 找到对应模型文件
        weights = pickle.load(fin)
